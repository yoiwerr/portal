...
├── .gitignore
│
├── nginx/default.conf             ← 线上路由: / → 首页, /chatlab → Streamlit, /api → FastAPI, /smooth → MakeItSmooth
...
│
└── MakeItSmooth/                  ← 子项目: AI 工作流增强 Agent
    ├── app.py                     ← FastAPI 入口
...
| MakeItSmooth/ | 工作流增强 Agent 代码 | 写 Skill、调追问策略 |
...
拉起 ChatLab FastAPI (:8000) + ChatLab Streamlit (:8501) + MakeItSmooth (:8001)，`Ctrl+C` 全停。
...
访问 `http://<公网IP>`。nginx 层路由: `/` 首页, `/chatlab` Streamlit, `/api` FastAPI, `/smooth` MakeItSmooth。
...
1. **MakeItSmooth 集成** — 将 MakeItSmooth 工作流增强 Agent 作为子项目加入 Portal
2. **架构对齐 ChatLab** — FastAPI + LangChain Agent + @tool，LLM 层从 ~440 行简化为 40 行 ChatOpenAI 工厂
3. **Docker 化** — Dockerfile + docker-compose.yml，smooth-api 容器 + volume 持久化
4. **Nginx 路由** — 新增 5 个 location 块：`= /smooth`（尾部斜杠重定向），`/smooth/css/`、`/smooth/js/`（静态资源代理），`/smooth/api/`（SSE 流式代理），`/smooth/`（首页代理）
5. **首页卡片** — 新增 MakeItSmooth 项目卡片（⚡ New badge）
6. **子路径部署修复** — 前端全部改用相对路径：HTML `href="css/style.css"` + `src="js/..."`, JS `fetch('api/chat/stream')`（去掉前导 `/`），避免与 portal 层的 `/css/` `/js/` `/api/` 冲突
7. **Docker 服务名冲突修复** — MakeItSmooth 服务名 `api` → `smooth-api`，解决与 ChatLab 同名 `api` 服务在 docker compose include 时的冲突
8. **模型待配** — SGLang base URL + model 通过 `.env` 注入，明天再配

### 2026-07-03

1. **run_dev.py 三服务** — 新增 MakeItSmooth (:8001) 到本地启动器，与 ChatLab FastAPI (:8000) + Streamlit (:8501) 并列
2. **首页路由补全** — ChatLab FastAPI 新增 `/smooth` → 302 重定向到 `localhost:8001`，本地开发点首页卡片即可跳转
3. **端口配置化** — MakeItSmooth `config.py` 新增 `API_HOST` / `API_PORT` 环境变量，app.py `__main__` 块使用配置值
